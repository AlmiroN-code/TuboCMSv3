...
# Ensure Redis is configured for persistence and proper memory
cat > /etc/redis/redis.conf.d/tubecms.conf << 'EOF' 2>/dev/null || true
maxmemory 256mb
...
if ! redis-cli ping > /dev/null 2>&1; then
    error "Redis is not running. Please check Redis installation."
fi
