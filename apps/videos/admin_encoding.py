...
class VideoEncodingProfileAdmin(admin.ModelAdmin):
    list_display = ('name', 'resolution', 'width', 'height', 'bitrate', 'is_active', 'order')
...
class MetadataExtractionSettingsAdmin(admin.ModelAdmin):
    list_display = ('__str__', 'is_active', 'poster_width', 'poster_height', 'preview_duration')
    
    fieldsets = (
        ('Настройки постера', {
            'fields': ('poster_width', 'poster_height', 'poster_format', 'poster_quality')
        }),
        ('Настройки превью', {
            'fields': ('preview_width', 'preview_height', 'preview_duration', 'preview_segment_duration', 'preview_format', 'preview_quality')
        }),
        ('Общие настройки', {
            'fields': ('is_active',)
        }),
