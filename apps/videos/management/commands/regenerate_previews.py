...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if video_id:
            ...
            videos = Video.objects.filter(converted_files__isnull=False).exclude(converted_files=[])
        ...
        for video in videos:
            ...
            try:
                if not video.temp_video_file:
                    self.stdout.write(self.style.WARNING(f'  No temp video file for video {video.id}'))
                    continue
                
                if not os.path.exists(video.temp_video_file.path):
                    self.stdout.write(self.style.WARNING(f'  Temp file does not exist: {video.temp_video_file.path}'))
                    continue
                ...
                if VideoProcessingService.extract_poster(video_path, poster_path, settings_obj):
                    ...
                    self.stdout.write(self.style.ERROR(f'  Failed to generate poster for video {video.id}'))
                ...
                if VideoProcessingService.extract_preview(video_path, preview_path, settings_obj):
                    ...
                    self.stdout.write(self.style.ERROR(f'  Failed to generate preview for video {video.id}'))
                
                # Save video
                video.save()
                
                self.stdout.write(self.style.SUCCESS(f'  Successfully regenerated preview for video {video.id}'))
                processed_count += 1
