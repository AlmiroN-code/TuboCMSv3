...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if force:
            ...
            videos = Video.objects.filter(converted_files__isnull=True) | Video.objects.filter(converted_files=[])
        ...
        self.stdout.write(f'Using {profiles.count()} encoding profiles: {list(profiles.values_list("name", flat=True))}')
        ...
        for video in videos:
            ...
            try:
                ...
                if success:
                    ...
                    self.stdout.write(self.style.ERROR(f'  Failed to process video {video.id}'))
                    error_count += 1
