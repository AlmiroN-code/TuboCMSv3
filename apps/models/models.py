...
class Model(models.Model):
    ...
    # Display info
    display_name = models.CharField(max_length=100, help_text="Stage name or display name")
    ...
    # Career info
    career_start = models.DateField(blank=True, null=True, help_text="Career start date")
    ...
    tattoos_description = models.TextField(blank=True, help_text="Description of tattoos")
    has_piercings = models.BooleanField(default=False)
    piercings_description = models.TextField(blank=True, help_text="Description of piercings")
    ...
    breast_size = models.CharField(max_length=20, choices=BREAST_SIZE_CHOICES, blank=True)
    measurements = models.CharField(max_length=50, blank=True, help_text="Body measurements (e.g., 32A-60-83)")
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            try:
                transliterated_name = transliterate.translit(self.display_name, 'ru', reversed=True)
...
class ModelVideo(models.Model):
    ...
    model = models.ForeignKey(Model, on_delete=models.CASCADE, related_name='model_videos')
    video = models.ForeignKey('videos.Video', on_delete=models.CASCADE, related_name='video_models')
    is_primary = models.BooleanField(default=False, help_text="Primary model in this video")
    created_at = models.DateTimeField(auto_now_add=True)
...
class ModelSubscription(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='model_subscriptions')
    model = models.ForeignKey(Model, on_delete=models.CASCADE, related_name='subscribers')
    created_at = models.DateTimeField(auto_now_add=True)
