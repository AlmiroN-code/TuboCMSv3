...
class ModelAdmin(admin.ModelAdmin):
    form = ModelForm
    list_display = ('display_name', 'user', 'gender', 'age', 'country', 'is_verified', 'is_active', 'views_count', 'subscribers_count', 'videos_count', 'created_at')
    list_filter = ('gender', 'is_verified', 'is_active', 'is_premium', 'hair_color', 'eye_color', 'country', 'created_at')
    search_fields = ('display_name', 'user__username', 'user__email', 'bio', 'country', 'ethnicity')
    readonly_fields = ('views_count', 'subscribers_count', 'videos_count', 'likes_count', 'created_at', 'updated_at', 'avatar_display', 'cover_photo_display')
    filter_horizontal = ()
    
    fieldsets = (
        ('Основная информация', {
            'fields': ('user', 'display_name', 'slug', 'bio', 'avatar', 'cover_photo')
        }),
        ('Личная информация', {
            'fields': ('gender', 'age', 'birth_date', 'country', 'ethnicity', 'career_start', 'zodiac_sign')
        }),
        ('Физические характеристики', {
            'fields': ('hair_color', 'eye_color', 'has_tattoos', 'tattoos_description', 'has_piercings', 'piercings_description')
        }),
        ('Размеры', {
            'fields': ('breast_size', 'measurements', 'height', 'weight')
        }),
        ('Статус', {
            'fields': ('is_verified', 'is_active', 'is_premium')
        }),
        ('Статистика', {
            'fields': ('views_count', 'subscribers_count', 'videos_count', 'likes_count'),
            'classes': ('collapse',)
        }),
        ('Даты', {
    ...
    def cover_photo_display(self, obj):
        if obj.cover_photo:
            return format_html('<img src="{}" width="200" height="100" style="object-fit: cover;" />', obj.cover_photo.url)
