...
class AdCampaignListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return AdCampaign.objects.filter(advertiser=self.request.user).order_by('-created_at')
...
class AdBannerListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return AdBanner.objects.filter(campaign__advertiser=self.request.user).select_related('campaign', 'placement').order_by('-created_at')
...
def ad_stats_api(request):
    ...
    stats = {
        ...
        'total_impressions': AdImpression.objects.filter(banner__campaign__in=campaigns).count(),
