...
class AdPlacement(models.Model):
    ...
    placement_type = models.CharField(max_length=20, choices=PLACEMENT_TYPES, verbose_name="Тип размещения")
...
class AdCampaign(models.Model):
    ...
    advertiser = models.ForeignKey(User, on_delete=models.CASCADE, related_name='ad_campaigns', verbose_name="Рекламодатель")
    description = models.TextField(blank=True, verbose_name="Описание")
    ...
    target_impressions = models.PositiveIntegerField(null=True, blank=True, verbose_name="Целевые показы")
    target_clicks = models.PositiveIntegerField(null=True, blank=True, verbose_name="Целевые клики")
    is_active = models.BooleanField(default=True, verbose_name="Активно")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Создано")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Обновлено")
...
class AdBanner(models.Model):
    ...
    campaign = models.ForeignKey(AdCampaign, on_delete=models.CASCADE, related_name='banners', verbose_name="Кампания")
    placement = models.ForeignKey(AdPlacement, on_delete=models.CASCADE, related_name='banners', verbose_name="Место размещения")
    banner_type = models.CharField(max_length=20, choices=BANNER_TYPES, verbose_name="Тип баннера")
    
    # Content fields
    image = models.ImageField(upload_to='ads/images/', blank=True, null=True, verbose_name="Изображение")
    video = models.FileField(upload_to='ads/videos/', blank=True, null=True, verbose_name="Видео")
    ...
    # Tracking
    impressions_count = models.PositiveIntegerField(default=0, verbose_name="Количество показов")
...
class AdImpression(models.Model):
    ...
    banner = models.ForeignKey(AdBanner, on_delete=models.CASCADE, related_name='impressions', verbose_name="Баннер")
...
class AdClick(models.Model):
    ...
    banner = models.ForeignKey(AdBanner, on_delete=models.CASCADE, related_name='clicks', verbose_name="Баннер")
    ip_address = models.GenericIPAddressField(verbose_name="IP адрес")
    user_agent = models.TextField(blank=True, verbose_name="User Agent")
    referer = models.URLField(blank=True, verbose_name="Реферер")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Создано")
...
class AdZone(models.Model):
    ...
    slug = models.SlugField(max_length=100, unique=True, verbose_name="Слаг")
    description = models.TextField(blank=True, verbose_name="Описание")
    placements = models.ManyToManyField(AdPlacement, related_name='zones', verbose_name="Места размещения")
    is_active = models.BooleanField(default=True, verbose_name="Активно")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Создано")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Обновлено")
